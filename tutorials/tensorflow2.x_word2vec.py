...
with tf.device('/cpu:0'):
  ...
  embeddings = tf.Variable(
      tf.random.normal([vocabulary_size, embedding_size], -1.0, 1.0))
...
for step in xrange(num_steps):
  train_inputs, train_labels = generate_batch(
    ...
    if step > 0:
      loss = tf.reduce_mean(
            tf.nn.nce_loss(weights=nce_weights,
                           biases=nce_biases,
                           labels=train_labels,
                           inputs=tf.nn.embedding_lookup(embeddings, train_inputs),
                           num_sampled=num_sampled,
                           num_classes=vocabulary_size))
    # The average loss is an estimate of the loss over the last 2000 batches.
      print("step %i Average loss:%f " % (step, loss))
  ...
  if step % 10000 == 0:
    ...
    sim = tf.matmul(
      valid_embeddings, normalized_embeddings, transpose_b=True).numpy()
